    ...
    <>
      ...
      <div id="receipt" className="mx-auto my-5 max-w-4xl space-y-4 border border-slate-200 bg-white p-6 text-slate-900 shadow-sm">
        ...
        <div className="border border-slate-200 bg-slate-50 p-3">
          ...
          {genError && (
            <p className="mb-2 rounded border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{genError}</p>
          )}
          {generating ? (
            <div className="flex items-center gap-2 py-4 text-xs text-slate-500">
              <Loader2 className="h-4 w-4 animate-spin" />
              Generating packet QR codes...
            </div>
          ) : packetCodes.length === 0 ? (
            <p className="text-xs text-slate-500">
              No packet QR codes yet. Complete all gate steps so codes are auto-generated.
            </p>
          ) : (

...
const nav = [
  { label: "Overview", href: "/delivery-distributor" },
  { label: "Assigned Orders", href: "/delivery-distributor/orders", badge: "0" },
  { label: "Pickup Requests", href: "/delivery-distributor/pickup" },
  { label: "Active Deliveries", href: "/delivery-distributor/active" },
  { label: "Confirm Delivery", href: "/delivery-distributor/confirm" },
  { label: "Delivery History", href: "/delivery-distributor/history" },
];

export default function DeliveryDistributorLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      <div className="mb-6 flex items-center justify-between px-3">
        <p className="text-xs font-semibold uppercase tracking-widest text-violet-500">Delivery Distributor</p>
        <button
          type="button"
      ...
      <nav className="flex-1 space-y-1">
        {nav.map((link) => {
          const active =
            pathname === link.href ||
            (link.href !== "/delivery-distributor" && pathname.startsWith(link.href));
          return (
            <Link
              key={link.href}
              href={link.href}
              ...
              className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold transition
                ${active ? "bg-violet-50 text-violet-700" : "text-slate-700 hover:bg-slate-50 hover:text-slate-900"}`}
            >
              {link.label}
              {link.badge && (
  ...
  return (
    <div className="flex min-h-[calc(100vh-80px)]">
      <button type="button" onClick={() => setMobileOpen((v) => !v)}
        className="fixed bottom-5 right-5 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-violet-500 text-white shadow-lg lg:hidden" aria-label="Toggle menu">
